...
export default function DisciplineSearchPage({ props }) {
    ...
    let fetchData = async (Params = { discipine: '', degree_type: [], attendance: [], format: [], duration: [], tuition: '', countries: [] }) => {
        ...
        let url;
        if (discipline) {
            url = baseUrl + `discipline=${discipline}&page=${currentPage + 1}&`
            console.log(url)
        }
    ...
    }, [ searchParam, isChecked, attendanceChecked, disciplinesList, searchParams, currentPage ]);
                        ...
                        <div className="" >
                            ...
                            <div className={showInfo ? 'block py-4' : 'hidden'}>
                                {disciplines.map((discipline) => (
                                    <Link
                                        key={discipline.id}
                                        to={`/search-discipline?discipline=${discipline.slug}`}
            ...
            <aside className="hidden"></aside>
                ...
                <div className=" flex w-full">
                    <div className="mb-4 w-full px-4">
                        <h1 className="text-3xl w-fit">{  `"${discipline.replaceAll('-', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}"`} Courses</h1>
